...
export class WareHouseComponent implements OnInit {
  ...
  submit!: string;
  cancel!: string;
  clickedRows = new Set<WareHouseModel>();
  ...
  indexes!: any[];
    displayedColumns: string[] =
        ['select','WareHouseName', 'Description', 'Location'];
  ...
  ngOnInit() {
    this.titleService.setTitle("Warehouse - Tasker");
      this.refreshMe();
  }
  ...
  refreshMe() {
    ...
    this._cf.getPageData('WareHouse', this.pScreenId, this._auth.getUserId(), this.pTableId,
  ...
  onView = (id: number) => {
    ...
    this.warehouseservice.getWareHouseEntry(id).subscribe((result: WareHouseModel) => {
      this._ui.loadingStateChanged.next(false);
      result.entryMode = 'V';
  ...
  onId(id: number, row:WareHouseModel) {
  ...
  openEntry  (result: WareHouseModel) {
    if (result === undefined) {
      const dialogRef = this.dialog.open(WareHouseEntryComponent, {
        disableClose: true,
        data: {}
      });
