...
export class ProductStockService {
   ...
   getProductStockEntry(id: number): Observable<ProductStockModel> {
      return this.httpClient.get<ProductStockModel>(this._globals.baseAPIUrl + 'ProductStock/' + id).pipe(
   ...
   getProductStockSubmit(data: ProductStockModel) {
      data.auditColumns = {
      'userId': 1,
      'hostname': 'test',
      'ipaddress': 'test',
      ...
      switch (data.entryMode) {
          ...
          case 'A': {
          return this.http.post(this._globals.baseAPIUrl + 'ProductStock/create', data, this._cf.requestOptions()).pipe(
          ...
          case 'E': {
          return this.http.post(this._globals.baseAPIUrl + 'ProductStock/edit', data, this._cf.requestOptions()).pipe(
          map((response: Response) => {
          return response.json();
          }), catchError(this._cf.handleError));
          }
          ...
          case 'D': {
          return this.http.post(this._globals.baseAPIUrl + 'ProductStock/delete', data, this._cf.requestOptions()).pipe(
          map((response: Response) => {
          return response.json();
          }), catchError(this._cf.handleError));
          }
