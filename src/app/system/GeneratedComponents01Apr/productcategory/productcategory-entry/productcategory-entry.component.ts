...
import { ProductCategoryEntryService } from './productcategory-entry.service';
import { Direction } from '@angular/cdk/bidi';

@Component({
  selector: 'app-productcategory-entry',
  templateUrl: './productcategory-entry.component.html',
  styleUrls: ['./productcategory-entry.component.scss']
})

export class ProductCategoryEntryComponent implements OnInit {
    url!: string;
  ...
  constructor(
      ...
      private dapiService: ProductCategoryEntryService,
      private _msg: MessageBoxService,
      private _globals: AppGlobals,
      private _auth: AuthService,
      private _select: SelectService,
      private _myService: ProductCategoryService,
      private dialogRef: MatDialogRef<ProductCategoryEntryComponent>,
  ...
  ngOnInit() {
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.dapiService.productCatControllers(this.pModel).subscribe(res => {
    ...
    for(let i=0;i<=this.data.length;i++){
     ...
     console.log(this.last);
          ...
          if(this.last.records[0].entryMode == "A"){
           ...
           this.dapiService.productCatEntryA(this.last).subscribe(nexto => {
             this.res = nexto;
             if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
           ...
           this.dapiService.productCatEntryE(this.last).subscribe(nexto => {
             this.res = nexto;
             if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
              this._msg.showInfoSuccess("Message", "Saved succesfully");
            this.dialogRef.close();
