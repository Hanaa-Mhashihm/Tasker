...
import { ProductGroupEntryService } from './productgroup-entry.service';
import { Direction } from '@angular/cdk/bidi';

@Component({
  selector: 'app-productgroup-entry',
  templateUrl: './productgroup-entry.component.html',
  styleUrls: ['./productgroup-entry.component.scss']
})

export class ProductGroupEntryComponent implements OnInit {
    url!: string;
  ...
  constructor(
    private dapiService: ProductGroupEntryService,
      private _ui: UIService,
      private _msg: MessageBoxService,
      private _globals: AppGlobals,
      private _auth: AuthService,
      private _select: SelectService,
      private _myService: ProductGroupService,
      private dialogRef: MatDialogRef<ProductGroupEntryComponent>,
  ...
  ngOnInit() {
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.dapiService.productGroupControllers(this.pModel).subscribe(res => {
    ...
    for(let i=0;i<=this.data.length;i++){
     ...
     console.log(this.last);
          ...
          if(this.last.records[0].entryMode == "A"){
           ...
           this.dapiService.productGroupEntryA(this.last).subscribe(nexto => {
             this.res = nexto;
             if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
           ...
           this.dapiService.productGroupEntryE(this.last).subscribe(nexto => {
             this.res = nexto;
             if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
              this._msg.showInfoSuccess("Message", "Saved succesfully");
            this.dialogRef.close();
