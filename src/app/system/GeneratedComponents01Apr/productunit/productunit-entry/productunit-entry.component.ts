...
import { ProductUnitEntryService } from './productunit-entry.service';
import { Direction } from '@angular/cdk/bidi';

@Component({
  selector: 'app-productunit-entry',
  templateUrl: './productunit-entry.component.html',
  styleUrls: ['./productunit-entry.component.scss']
})

export class ProductUnitEntryComponent implements OnInit {
    url!: string;
  ...
  constructor(
    private dapiService: ProductUnitEntryService,
      private _ui: UIService,
      private _msg: MessageBoxService,
      private _globals: AppGlobals,
      private _auth: AuthService,
      private _select: SelectService,
      private _myService: ProductUnitService,
      private dialogRef: MatDialogRef<ProductUnitEntryComponent>,
  ...
  ngOnInit() {
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.dapiService.productUnitControllers(this.pModel).subscribe(res => {
    ...
    for(let i=0;i<=this.data.length;i++){
     ...
     console.log(this.last);
          ...
          if(this.last.records[0].entryMode == "A"){
           ...
           this.dapiService.productUnitEntryA(this.last).subscribe(nexto => {
             this.res = nexto;
             if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
           ...
           this.dapiService.productUnitEntryE(this.last).subscribe(nexto => {
             this.res = nexto;
             if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
              this._msg.showInfoSuccess("Message", "Saved succesfully");
            this.dialogRef.close();
