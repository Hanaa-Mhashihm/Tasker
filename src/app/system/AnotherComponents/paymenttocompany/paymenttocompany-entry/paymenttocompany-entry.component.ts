...
export class PaymentToCompanyEntryComponent implements OnInit {
      ...
      this.dapiService.Controllers(this.pModel).subscribe(res => {
        ...
        for(let k=0;k<=this.dropList.length;k++) {
          this.dropItem = this.dropList[k]
            ...
            if(this.dropList[k].tableColumnId==310){
              ...
              this._select.getDropdown("CustomeraccountId", "Customeraccount", "AccountName", str, false).subscribe((resu: SelectModel[]) => {
                ...
                for(let j=0;j<=this.dropList.length;j++) {
                  if(this.dropList[j].tableColumnId==311){
  ...
  deleteFun4(id: number) {
    ...
    onResults4(id:number, e:any, i:number) {
      console.log('ee',e);
      this.last.child3[i].records.forEach((res:any) => {
        if (res.tableColumnId === id) {
          console.log('ee', e);
          res.value = e.toString()
            this.onChange1(res.value, i)
    ...
    onResults5(id:number, e:any, i:number) {
      console.log('ee',e);
      this.last.child4[i].records.forEach((res:any) => {
        if (res.tableColumnId === id) {
          console.log('ee', e);
          res.value = e.toString()
            this.onChange1(res.value, i)
