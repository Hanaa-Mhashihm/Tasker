...
export class InvoiceEntryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.subTotalE = "المجموع الفرعي"
      this.totalE = "المبلغ الإجمالي"
      this.addItem =  "+ إضافة عنصر"
    ...
    this.dapiService.Controllers(this.pModel).subscribe(res => {
      ...
      for(let k=0;k<=this.dropList.length;k++) {
        ...
        console.log('container', this.container, this.dropList, k)
        if(this.dropList[k].tableColumnId==284){
          let str= "active=1 and deleted=0 and customerid="+this.dropList[k].value
          this._select.getDropdown("CustomeraccountId", "Customeraccount", "AccountName", str, false).subscribe((resu: SelectModel[]) => {
            console.log(resu,'yeees');
            for(let j=0;j<=this.dropList.length;j++) {
              if(this.dropList[j].tableColumnId==287){
              this.dropList[j].myarray = resu;
              }
            }
          })
        }
  ...
  onParent(){}
  customerAcc(){
    console.log('in cus acc');
    
    for(let j=0;j<=this.data.length;j++) {
      console.log(this.data[j].value,'tt');
      
      if(this.data[j].tableColumnId==284){
        if(Number(this.data[j].value)>1){
          this._select.getDropdown(this.data[j].refId, this.data[j].refTable, this.data[j].refColumn, this.data[j].refCondition+this.data[j].value, false).subscribe((res: SelectModel[]) => {
            console.log(res,'yeees');
  ...
  deleteFun2(id: number) {
    ...
    onResults2(id:number, e:any, i:number) {
      console.log('ee',e);
      // for(let g=0;g<this.last.child1.length;g++)
    ...
    onResults3(id:number, e:any, i:number) {
      console.log('ee',e);
      
      this.last.child2[i].records.forEach((res:any) => {
        if (res.tableColumnId === id) {
          console.log('ee', e);
          
          res.value = e.toString()
          if(res.tableColumnId !== 28) {
            this.onChangeTax(res.value, i)
          }
          ...
          if(this.lastDark.records[0].entryMode == "A"){
           ...
           this.dapiService.EntryE(this.lastDark).subscribe(nexto => {
            ...
            this.dialogRef.close();
              ...
              this._msg.showInfoSuccess("رسالة", "تم حفظ بنجاح");
            this.dialogRef.close();
            }
