...
expenseFilingTaxDataSource = new MatTableDataSource(this.expenseFilingTaxData);
  ...
  ngOnInit() {
    ...
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.expenseCode2= "Expense code"
      this.itemE = "Item"
      this.amountE = "Amount"
      ...
      this.expenseCode2 = "كود المصاريف"
      this.itemE = "مصروف"
...
this._select.getDropdown('miscdetailid','miscdetail','misctext','miscid=18',false).subscribe((res: SelectModel[]) => {
  ...
  onResults(model:any, e:any, drop:any,i?:number) {
    ...
    }else if(drop==='fromAccount'){
      model.fromAccount=e
    }else if(drop==='toAccount'){
      model.toAccount=e
      // this.onChangeAccount1(e, i!)
    }else if(drop==='paymentType'){
      model.paymentType=e
    }else if(drop==='tax'){
      model.taxId=e
      this.onChangeDrop(e)
