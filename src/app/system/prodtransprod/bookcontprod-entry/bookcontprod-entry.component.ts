...
import { BookContProdEntryService } from './bookcontprod-entry.service';
import { Direction } from '@angular/cdk/bidi';
@Component({
  selector: 'app-bookcontprod-entry',
  templateUrl: './bookcontprod-entry.component.html',
  styleUrls: ['./bookcontprod-entry.component.scss']
})

export class BookContProdEntry2Component implements OnInit {
  ...
  constructor(
	  private dapiService: BookContProdEntryService,
      private _ui: UIService,
      private _msg: MessageBoxService,
      private _auth: AuthService,
      private _globals: AppGlobals,
      private _select: SelectService,
      private dialogRef: MatDialogRef<BookContProdEntry2Component>,
  ...
  ngOnInit() {
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      console.log(this.pModel,'arrrrr')
      this.dapiService.Controllers(this.pModel.array).subscribe(res => {
        this._ui.loadingStateChanged.next(false);
        this.data = res;
        console.log(this.data[1], '1', this.data[2], '2')
        console.log(this.data);
        
        this.data[1].value= (this.pModel.prodTransProdId).toString()
        this.data[2].value= (this.pModel.productId).toString()
  ...
  onChangeValueC(id:number){
    ...
    console.log("working fine",this.stringOfV)
    for(let k=0;k<=this.dropList.length;k++) {
      
      if(this.dropList[k].tableColumnId == 748) {
        this.dropItem = this.dropList[k]
