...
import { forexRateModel, productPricingModel } from '../suppinvoice.model';
...
export class SuppInvoiceEntryComponent implements OnInit {
  ...
  addChild1ExpenseItem(id:number) {  
    ...
    this.dapiService.child1Controllers(this.model2).subscribe((res) => {
      ...
      this.dropListItem.push(this.childElemInit[9])
        ...
        console.log("DropitemTax", this.dropItemchild)
            ...
            if(this.dropItemchild.tableColumnId == 615) {
              ...
              this._select.getDropdown(this.dropItemchild.refId, this.dropItemchild.refTable, this.dropItemchild.refColumn, this.dropItemchild.refCondition, false).subscribe((res: SelectModel[]) => {
                console.log("drop: ", res, k);
                this.dropListItem[k].myarray = res;

                // res.map(item => {
                  
                //   console.log('this.childElemInit', this.childElemInit, item.id.toString(),res);
                //   if (item.id.toString() === this.childElemInit[4].value){
                //     this.childElemInit[4].myarray2= item.name;
                //     console.log('myarray2', this.childElemInit[4].myarray2);
              ...
              this._select.getDropdown(this.dropItemchild.refId, this.dropItemchild.refTable, this.dropItemchild.refColumn, this.dropItemchild.refCondition, false).subscribe((res: SelectModel[]) => {
                ...
                res.forEach(item=> {
                  ...
                  if (item.id.toString() === this.childElemInit[4].value){
                        this.dropListItem[k].myarray2= item.name;
                        // console.log('myarray2', this.dropListItem[k].myarray2);
                      }
                })
                this.onChangeValue(+this.dropListItem[k].value, (k/2) )
              });
  ...
  onResults2(id:number, e:any, i:number) {
    ...
    this.last.child1[i].records.forEach((res:any) => {
      if (res.tableColumnId === id) {
        ...
        }else if(res.tableColumnId != 615 && res.tableColumnId != 622) {
  ...
  onChangeCurrency(idC: number){
    ...
    this.dapiService.getForexRate(idC, this.data[2].value).subscribe((resu: forexRateModel) => {
      this._ui.loadingStateChanged.next(false);
      console.log(resu)
      this.data[11].value = resu.name
  ...
  onChangeDate(e: Event){
    ...
    this.dapiService.getForexRate(+this.data[6].value, idD.toString()).subscribe((resu: forexRateModel) => {
      this._ui.loadingStateChanged.next(false);
      console.log(resu)
      this.data[11].value = resu.name
    })
  ...
  deleteFun2(id: number) {
    ...
    onSubmit() {
      ...
      for (let i = 0; i < this.lastDark.child2.length; i++) {
       ...
       console.log("Dark",this.lastDark);
       console.log(JSON.stringify(this.lastDark), "dark33")
