...
export class CompanyBankEntry3Component implements OnInit {
  url!: string;
  ...
  constructor(
    ...
    private upload: UploadService,
      ...
      private dialogRef: MatDialogRef<CompanyBankEntry3Component>,
  ...
  ngOnInit() {
    ...
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.dapiService.companyBankControllers(this.pModel).subscribe(res => {
  ...
  if (id=== 132){
    this._select.getDropdown('customerid','customer','customername','active=1 and deleted=0 and customerid>1',false).subscribe((res: SelectModel[]) => {
      ...
      for(let i=0;i<=this.light.length;i++){
        if (this.light[i].tableColumnId===1018){
