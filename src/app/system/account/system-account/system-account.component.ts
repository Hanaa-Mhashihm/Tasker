...
export class SystemAccountComponent implements OnInit {
  ...
  // });
    ...
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.submit = "Submit"
      this.cancel = "Cancel"
      this.subAccount = "This is a sub-account"
    ...
    this.dapiService.accountControllers(this.pModel).subscribe(res => {
      ...
      this.data.forEach((ala2) => {
        if(ala2.tableColumnId == 471) {
          if(Number(ala2.value) > 1) {
            this.checkedIsSub =true
            this.numval=ala2.value
          }
        }
      })
      for(let j=0;j<this.data.length;j++){
        if(this.data[j].tableColumnId == 472) {
          // console.log(this.data[j].refCondition+this.typeValue)
          // this.data[j].refCondition=this.data[j].refCondition+this.typeValue
        }
      }
      ...
      window.innerWidth <= 680
        ...
        for(let k=0;k<=this.dropList.length;k++) {
          ...
          if( this.dropList[k] && this.dropList[k].tableColumnId == 472) {
            ...
            console.log(this.dropItem.refCondition + this.numval);
    ...
    for(let i=this.light.length-1;i>=0;i--){
      ...
      if(x == 1) {
        ...
        this.dapiService.generateCode(this.val11, id.toString()).subscribe(res => {
          ...
          this.resVal = res
          console.log("VV", res)
          this.light[2].value = this.resVal[0].name
          console.log("VV", this.ver5)
        })
      }
    ...
    onResults(id:number, e:any) {
      ...
      this.light.forEach((res:any) => {
        if (res.tableColumnId === id) {
          ...
          if(res.tableColumnId === 471) {
            this.onChangeValue(res.value)
          } else if(res.tableColumnId === 472) {
            this.onChangeValue2(res.value)
          } 
    ...
    if(this.last.records[2].value != "Error") {
      if(this.last.records[0].entryMode == "A"){
        ...
        this.dapiService.accountEntryE(this.last).subscribe(nexto => {
          ...
          if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
            this._msg.showInfoError("Error", "Invalid Account Code!!");
          
          }else if(localStorage.getItem(this._globals.baseAppName + '_language') == "16002") {
            
            this._msg.showInfoError("خطاء", "رمز الحساب غير صحيح");
