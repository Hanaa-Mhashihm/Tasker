     ...
     this.last.records.forEach((Object3:any)=> {
      ...
      if(Object3.tableColumnId == 45){
        if(Object3.value >= 1 && this.maxDate>=this.minDate) {
          ...
          if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
            if(Object3.value < 1 && this.maxDate>=this.minDate) {
              this._msg.showInfoError("Error!!", "Amount is less than 1");
            }else if(Object3.value >= 1 && this.maxDate<this.minDate) {
              this._msg.showInfoError("Error!!", "To date cann't be less than From date");
            }else if(Object3.value < 1 && this.maxDate<this.minDate) {
              this._msg.showInfoError("Error!!", "Amount is less than 1 and To date cann't be less than From date");
            }
            
          this.last.records = []
          }else if(localStorage.getItem(this._globals.baseAppName + '_language') == "16002") {
            if(Object3.value < 1 && this.maxDate>=this.minDate) {
              this._msg.showInfoError("خطأ!!", "الكمية اقل من واحد");
            }else if(Object3.value >= 1 && this.maxDate<this.minDate) {
              this._msg.showInfoError("خطأ!!", "تحقق من التاريخ ");
            }else if(Object3.value < 1 && this.maxDate<this.minDate) {
              this._msg.showInfoError("و التار خطأ!!", "تحقق من الكمية و التاريخ");
            }
