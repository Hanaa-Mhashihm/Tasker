...
import { SuppInvProcEntryService } from './suppinvproc-entry.service';
import { forexRateModel } from '../../suppinvoice/suppinvoice.model';
import { Direction } from '@angular/cdk/bidi';

@Component({
  selector: 'app-suppinvproc-entry',
  templateUrl: './suppinvproc-entry.component.html',
  styleUrls: ['./suppinvproc-entry.component.scss']
})

export class SuppInvProcEntryComponent implements OnInit {
  ...
  constructor(
	  private dapiService: SuppInvProcEntryService,
      private _ui: UIService,
      private _msg: MessageBoxService,
      private _auth: AuthService,
      private _globals: AppGlobals,
      private _select: SelectService,
      private dialogRef: MatDialogRef<SuppInvProcEntryComponent>,
  ...
  onChangeCurrency(idC: number){
    ...
    this.dapiService.getForexRate(idC, this.data[3].value).subscribe((resu: forexRateModel) => {
      this._ui.loadingStateChanged.next(false);
      console.log(resu)
      this.data[12].value = resu.name
  ...
  onChangeDate(e: Event){
    ...
    this.dapiService.getForexRate(+this.data[5].value, idD.toString()).subscribe((resu: forexRateModel) => {
      this._ui.loadingStateChanged.next(false);
      console.log(resu)
      this.data[12].value = resu.name
    })
